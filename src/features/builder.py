...
def prepare_features(df: pd.DataFrame,
    ...
    # Step 1.5: Register pre-existing orderflow features
    # These columns come from the loader but aren't generated by "add_*" functions
    # so we need to capture them manually for the group_map
    orderflow_cols = ['bid_vol', 'ask_vol', 'max_bid_vol', 'max_ask_vol', 
                     'avg_bid_vol', 'avg_ask_vol', 'delta_vol', 'cvd']
    found_orderflow = [c for c in orderflow_cols if c in df_result.columns]
    
    if found_orderflow:
        group_columns.setdefault('orderflow', []).extend(found_orderflow)
        print(f"  -> Orderflow features found: {len(found_orderflow)}")
        print(f"     {', '.join(found_orderflow)}")
