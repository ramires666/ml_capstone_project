...
Problem is described in README briefly without much details (1 points)
...
EDA:
No EDA (0 points)
Basic EDA (looking at min-max values, checking for missing values) (1 points)
...
Trained only one model, no parameter tuning (1 points)
...
The logic for training the model is exported to a separate script (1 points)
Score Received: 1

Reproducibility:
Not possible to execute the notebook and the training script. Data is missing or it's not easily accessible (0 points)
It's possible to re-execute the notebook and the training script without errors. The dataset is committed in the project repository or there are clear instructions on how to download the data (1 points)


Model deployment:
Model is not deployed (0 points)
Model is deployed (with Flask, BentoML or a similar framework) (1 points)


Dependency and environment management:
No dependency management (0 points)
Provided a file with dependencies (requirements.txt, pipfile, bentofile.yaml with dependencies, etc.) (1 points)
...
Dockerfile is provided or a tool that creates a docker image is used (e.g. BentoML) (1 points)
...
Docs describe clearly (with code) how to deploy the service to cloud or Kubernetes cluster (local or remote) (1 points)
